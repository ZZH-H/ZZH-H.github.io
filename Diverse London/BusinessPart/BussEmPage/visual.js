...
$("#inputfile").change(function() {
  ...
  r.onload = function() {
    //data is stored in the result property of the object
...
function draw(data) {
  ...
  // The interval date for each data
    ...
    if (!long) {
      barEnter
        ...
        .style("fill", d => {
          ...
          if (!type_list.includes(d.type))
          {
            type_list.push(d.type);
            $('.namelist').append('<span class="discrete"><span class="swatch" style="background:'+ getColor(d) +';"></span><span class="label" style="color: rgb(0, 0, 0);">'+ d.type +'</span></span>');
          }
        })
        .transition()
